...
Provides two public functions:
    ...
    build_graph(project_path, out_path) -> dict
        ...
        Raises ImportError with install instructions if the package is absent.
        Deletes chat_action_graph.json and context-store.json as side-effect
        cleanup to prevent conflicts with EGC memory.

    extract_relevant_files(graph, query, history, top_n=10) -> list[dict]
        Scores graph file nodes by keyword overlap with (query + history)
        and returns the top-N files with their literal code content.
...
def _check_graperoot() -> None:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Then: pip install graperoot"
        )


# ── Suppressed side-effect files ─────────────────────────────────────────────

_SUPPRESS = ("chat_action_graph.json", "context-store.json")


def _suppress_graperoot_state(data_dir: Path) -> None:
    """Delete GrapeRoot session-state files to prevent conflicts with EGC."""
...
def build_graph(project_path: Path, out_path: Path) -> dict[str, Any]:
    ...
    Attempts to call ``graperoot.graph_builder`` as a Python module first.
    Falls back to the ``graph-builder`` console-script entry point if the
    module does not expose a callable ``run()`` function.
    ...
    Returns:
        Parsed info_graph.json as a dict.
    """
    _check_graperoot()

    out_path.parent.mkdir(parents=True, exist_ok=True)

    # ── Attempt 1: graperoot.graph_builder.scan() — clean Python API ─────────
    # scan(root) -> dict  (returns graph dict directly, no file I/O required)
    ...
    try:
        ...
        if callable(getattr(_gb, "scan", None)):
            ...
            # Persist to out_path so callers can cache/inspect it
            out_path.write_text(
    ...
    # ── Attempt 2: subprocess via python -m graperoot.graph_builder ──────────
    if graph is None:
        cmd = [
    ...
    # Suppress GrapeRoot session-state files (in the .dual-graph/ data dir)
    _suppress_graperoot_state(out_path.parent)

    logger.debug(
        "graph loaded: %s files, %s symbols, %s nodes, %s edges",
        graph.get("file_count", "?"),
        graph.get("symbol_count", "?"),
...
# ── Keyword-scored file extraction ────────────────────────────────────────────
...
def extract_relevant_files(
    ...
    """Return the top-N most relevant files from *graph* scored against the query.

    Scoring: count of keyword hits between (query + history) tokens and
    (file path tokens + symbol name tokens) for each file node.
    ...
    Args:
        ...
        project_path: Root path used to resolve relative file paths. Defaults
                      to cwd.

    Returns:
        List of dicts with keys: ``path`` (str), ``score`` (int), ``content`` (str).
    ...
    # Locate file nodes — graperoot stores them under various top-level keys
    ...
    # Fallback: if graph is a flat dict keyed by path
    ...
    for node in file_nodes:
        ...
        score = len(keywords & node_tokens)
